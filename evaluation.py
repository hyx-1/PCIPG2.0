...
def precision_score(predicted_complex, reference_complex, predicted_num):
    ...
    unmatch_pred_list = []
    number = 0
    for i, pred in tqdm(enumerate(predicted_complex)):
        overlapscore = 0.0
...
def recall_score(predicted_complex, reference_complex, reference_num):
    ...
    unmatch_pred_list = []
    c_number = 0
    for i, ref in tqdm(enumerate(reference_complex)):
        overlapscore = 0.0
        tmp_max_score_info = None
        ...
        if overlapscore > 0.2:
            ...
            if tmp_max_score_info is not None:
                match_info_list.append(tmp_max_score_info)
            if tmp_max_score_info['overlap_score'] >=0.8:
                true_info_list.append(tmp_max_score_info)
        else:
            unmatch_pred_list.append(pred)

    return c_number / (reference_num + 1e-4), c_number, match_info_list, unmatch_pred_list, true_info_list
...
def acc_score(predicted_complex, reference_complex):
    ...
    # ppv
    T_sum2 = 0.0
    B_sum = 0.0  # the number of proteins in reference complex
    for i in predicted_complex:
        ...
        for j in reference_complex:
            ...
            if len(overlap) > max:
                max = len(overlap)
        T_sum2 = T_sum2 + max
        B_sum = B_sum + len(set1)

    Sn = float(T_sum1) / float(N_sum)
    PPV = float(T_sum2) / float(B_sum)
...
def get_score(reference_complex, predicted_complex):
    ...
    precision, p_num, p_true, un_p = precision_score(predicted_complex, reference_complex, predicted_num)
    recall, r_num, r_true, un_r, true_info = recall_score(predicted_complex, reference_complex, reference_num)
    ...
    return  precision, recall, f1, acc,sn,PPV,msg + num_msg, p_true, r_true, true_info
