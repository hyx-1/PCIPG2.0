...
STRUCTURE_EMBEDDINGS_DIR.mkdir(parents=True, exist_ok=True)
...
def process_mpnn_embedding_fn(sample):
    # print('the input sample is', sample.keys())
    sample = transform_sample(sample)
    # print('the transform sample is', sample.keys())
    with torch.no_grad():
...
def write_pyd():
    for pdb_file in glob.iglob(str(PDB_DIR / '*.pdb')):#对应数据集的pdb
        # print(pdb_file)
        ...
        # print(record["visible_list"])
        record = process_mpnn_embedding_fn(record)
        with open(STRUCTURE_EMBEDDINGS_DIR / f'{save_name}.pyd', 'wb') as f:
